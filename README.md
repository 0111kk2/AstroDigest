...
X線天文・小型衛星・多波長観測・検出器分野の情報を1時間ごとに自動収集し、Claude API / Gemini API で日本語ダイジェストを作って通知するツールです。
...
6. **📄 arXiv 新着論文** — astro-ph.HE / astro-ph.IM / physics.ins-det の新着から、キーワード(X線・小型衛星・検出器など)関連度の高い論文を優先してアブストラクトを和訳

GitHub Actions が**1時間ごと**に実行されます。各回は直近 `HOURS_BACK`(既定2時間)の新着だけを見に行き、**新着が1件もなければファイルも通知も更新せず静かにスキップ**するので、1時間おきに実行しても「新着なし」でスパムになることはありません。新着があった回だけ、その日のページの先頭に更新時刻つきのブロックとして積み重なっていきます(1日の途中でも随時読める、ログ形式)。

結果は次のように保存・通知されます:

- **フィードサイト(既定・常時)**: `docs/data/` に保存されます。GitHub Pages を有効化すると、タイムライン型のページで新着のたびに積み重なり、スマホでスクロールして読めます(無料)
...
- GCN の取得はアーカイブページ(https://gcn.nasa.gov/circulars)の公開 JSON を利用しています。NASA 側のレート制限(429)にかかった場合はその回だけ「未掲載」として明示されます
