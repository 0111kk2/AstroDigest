...
3. **🇯🇵 国内X線天文・関連プレス** — ISAS/JAXA、JAXA、理研、東京大学、国立天文台の公開一覧から、XRISM・X線天文・関連検出器などの国内発表を抽出
4. **📄 arXiv 新着論文** — astro-ph.HE / astro-ph.IM / physics.ins-det の新着から、キーワード(X線・小型衛星・検出器など)関連度の高い論文を優先して要約
...
- `INCLUDE_DOMESTIC_PRESS`: `False` で国内プレスリリースセクションを無効化
- `PRESS_KEYWORDS`: 国内プレスリリース抽出用のキーワード
